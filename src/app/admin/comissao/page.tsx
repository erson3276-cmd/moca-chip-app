...
export default function ComissaoPage() {
  ...
  if (loading) return <div className="p-10 text-center text-gray-500 font-medium">Calculando comissões...</div>
    ...
    <div className="max-w-6xl space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Gestão de repasses para profissionais (Base: 50%).</p>
        </div>
        
        <div className="flex items-center gap-3 bg-[#121021] border border-white/5 p-1.5 rounded-2xl">
           <button className="p-2 hover:bg-white/5 rounded-xl text-gray-500 transition-all"><ChevronLeft size={18} /></button>
           ...
           <button className="p-2 hover:bg-white/5 rounded-xl text-gray-500 transition-all"><ChevronRight size={18} /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="p-8 rounded-[2.5rem] bg-[#121021] border border-white/5 space-y-4 shadow-2xl relative overflow-hidden group">
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-[#5E41FF]/10 rounded-full blur-3xl group-hover:bg-[#5E41FF]/20 transition-all" />
      ...
      <div className="bg-[#121021] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
         <div className="p-6 border-b border-white/5 flex items-center justify-between">
            <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400">Detalhamento de Repasses</h3>
            <button className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all">
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-white/5">
                    ...
                    <tr key={idx} className="hover:bg-white/[0.01] transition-colors group">
                       ...
                       <td className="p-6">
                          <div className="flex items-center gap-3">
                             ...
                             <span className="text-sm font-bold text-white">{sale.customers?.name || 'Cliente'}</span>
                          </div>
                       </td>
                       <td className="p-6">
                  ...
                  {filteredSales.length === 0 && (
                    <tr>
                       <td colSpan={5} className="p-20 text-center text-gray-600 font-medium italic">Nenhuma venda encontrada no período.</td>
                    </tr>
                  )}
               </tbody>
            </table>
